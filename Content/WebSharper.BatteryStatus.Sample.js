...
function Main(){
  ...
  const b=(this_1.h.push(new TextView("batterystatus", batteryStatus().View)),this_1);
...
function initializeBattery(){
  try {
    globalThis.navigator.getBattery().then((battery) => {
      ...
      return battery.addEventListener("dischargingtimechange", () => updateBatteryInfo(battery));
    });
  }
  catch(error){
    batteryStatus().Set(String(error.message));
  }
}
function batteryStatus(){
  return _c.batteryStatus;
}
function updateBatteryInfo(battery){
  const batteryText="Battery Level: "+String(toInt(battery.level*100))+"%<br>Charging: "+toSafe(battery.charging?"Yes \u26a1":"No")+"<br>Time to Full Charge: "+toSafe(battery.chargingTime>0?battery.chargingTime.toFixed(6)+"s":"N/A")+"<br>Time to Empty: "+toSafe(battery.dischargingTime>0?battery.dischargingTime.toFixed(6)+"s":"N/A");
  globalThis.document.getElementById("battery-status").innerHTML=batteryText;
}
...
let _c=Lazy((_i) => class $StartupCode_Client {
  ...
  static {
    this.batteryStatus=_c_1.Create_1("Checking battery status...");
  }
});
function NewGuid(){
